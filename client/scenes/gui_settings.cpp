...
void settings_streaming(const settings_context & ctx)
{
	...
	list.push_back({
	        .id = "##nxwarp_lz4_hc",
	        .label = _("Stronger lossless compression (LZ4 HC)"),
	        .description = _("Use LZ4 high compression when the server's direct NX Warp encoder has LZ4 enabled. It can reduce network bytes, but costs more CPU time on the server. Applies immediately."),
	        .ui = ui_kind::toggle,
	        .get_bool = [&config] { return config.nxwarp_lz4_hc; },
	        .set_bool = [&ctx, &config](bool v) { config.nxwarp_lz4_hc = v; config.save(); if (ctx.on_streaming_changed) ctx.on_streaming_changed(); },
	        .default_bool = default_config.nxwarp_lz4_hc,
	});

	list.push_back({
	        .id = "##nx_checkerboard",
	        .label = _("Checkerboard half-refresh (experimental)"),
	        .description = _("Direct NX Warp only. Alternates fresh samples each frame to reduce bandwidth. May shimmer in motion. Reconnect to apply."),
	        .ui = ui_kind::toggle,
	        .get_bool = [&config] { return config.nx_checkerboard; },
	        .set_bool = [&config](bool v) { config.nx_checkerboard = v; config.save(); },
	        .default_bool = default_config.nx_checkerboard,
	        .enabled = [&ctx] { return not ctx.in_game; },
	        .disabled_tooltip = disconnect_tip,
	});
