                ...
                contentItem: ColumnLayout {
                    ...
                    Kirigami.FormLayout {
                        ...
                        RowLayout {
                            ...
                            Kirigami.ContextualHelpButton {
                                toolTipText: i18n("How hard the encoder looks for the cheapest way to say each frame. Level 1 drops a coefficient whose error is worth less than the bits it saves. It costs no measurable encode time, but on rendered scenes it measured 0.1%% to 3.2%% more bytes for the same quality, so level 0 is the default. The level leaves no trace in the stream, so this line is the only place it can be read back.")
                            }
                        }
