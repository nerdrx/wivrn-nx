...
| `effort` | `0` | how hard the encoder looks for the cheapest way to say each frame. `0` is the plain dead-zone quantiser. `1` adds the **integer requantiser**: a coefficient quantised to ±1 whose squared error is worth less than the bits it saves is dropped. It costs no measurable encode time (9.12 → 9.18 ms a frame on RADV at 2 × 1088×1088, inside the run-to-run spread), **but it only pays on synthetic footage**: −2.4 % BD-rate on the pan fixture against **+0.1 % to +3.2 % on all five rendered vrroom clips**, on both entropy coders. The requantiser prices a dropped coefficient against the current frame only, while a coded tile's reconstruction is also the next frame's reference — dropping the pan fixture's synthetic noise leaves a cleaner reference and the gain compounds; dropping a rendered scene's specular detail and thin geometry degrades it and the loss compounds. `1` was the default until that was measured. It changes which levels are coded and nothing about how they decode — the stream carries no tool bit for it and no decoder can tell the levels apart, so `nxwarp_stream_stats::effort` is the only place the level in force can be read back — and both backends honour it. There is no level 2 yet: a wider motion search measures −0.05 % for +12 % encode time, and the one that does pay is the reference codec's integer trellis (−2.8 % to −10.7 %, negative on every fixture), which has no GPU implementation. Out of range is an error, not a fallback |
| `snap-identity` | `16` | **snap still tiles to a copy**, in 1/16 luma samples; 0 = off. When the head has barely moved -- every tile corner displaced by less than this -- the encoder sends the frame as if it had not moved at all, and the headset decodes those tiles as a straight copy instead of a filtered warp. On a Pico 4 that warp is 8.25 of 13.7 ms of Pass B per pair. The error introduced is at most **half the setting**: half a sample at 16, which is the rounding the motion search already accepts. Measured: **below 16 nothing ever snaps** (a head at rest still drifts ~0.57 samples a frame), at 16 about a third of still frames qualify for -0.05 dB and slightly FEWER bytes, and at ordinary head speeds nothing snaps at all. Measured live on a Pico 4 at rest, one session with it off and one with it at 16: decode **15.1 -> 11.4 ms per pair** (Pass B skip 8.35 -> 0.05 ms, the cost moving to the identity copy at 0.09 -> 4.17 ms), paced **50.6 -> 62.8 fps**, tiles 290 skip/288 coded -> 7/291, and the quantiser fell from a mean 35.8 to `min-qp` because the decode controller stopped binding at all -- the frames roughly double in size and that is the quality coming back. Needs `"backend": "vk"` and `"inter": "on"`; where those do not hold the DEFAULT quietly resolves to 0 and says so, while an explicit setting is still refused. Above 32 is refused, because the unit is sixteenths and past two samples the tool discards motion rather than rounding it |
...
effort**, on by default), `snap-identity` (as **Snap still tiles**, 1 sample), and
