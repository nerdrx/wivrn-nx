...
vec3 sample_block(uvec2 p, uint merged) {
    ...
    uint offset = (d & 0x3fffffffu) + (5u + merged) * ((q.y / 8u) * blocks_per_row + q.x / 8u);
    uint endpoints = blocks.block[offset + (merged * ((q.x ^ q.y) & 1u))];
    uint index = (q.y % 8u) * 8u + q.x % 8u;
    uint selector = (blocks.block[offset + 1u + merged + index / 16u] >> (2u * (index % 16u))) & 3u;
