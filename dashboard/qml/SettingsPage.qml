    ...
    ColumnLayout {
        ...
        Kirigami.FormLayout {
            ...
            RowLayout {
                ...
                Kirigami.ContextualHelpButton {
                    toolTipText: i18n("When the head has barely moved, send the frame as if it had not moved at all. Tiles that did not change then cost the headset a straight copy instead of a filtered warp, which is most of what its decode spends on a still scene. The picture error this can introduce is at most half the setting — half a sample at '1 sample', which is the same rounding the motion search already accepts.\n\nMeasured live on a Pico 4 at rest, the same session with it off and on: decode fell from 15.1 to 11.4 ms per eye pair, the frame rate rose from 50.6 to 62.8, and the quantiser dropped all the way back to the finest one allowed because the headset's decode stopped being the limit. Frames get bigger, and that is the sharpness returning. Moving scenes are unaffected — nothing snaps at ordinary head speeds. Needs the GPU encoder with inter prediction on, and is on at '1 sample' unless you turn it off.")
                }
            }
