...
bool nxwarp_decoder::on_stream_header(std::span<const uint8_t> header)
{
	...
	if (si.eyes > 1)
	{
		...
		if (compact_centre_active)
			nxvc_vk_decoder_plane_size(nxvc, 0, &extent.width, &extent.height);
		if (not planar_direct_active)
			rebuild_pool();
	}
	else
...
bool nxwarp_decoder::on_direct_stream_header(std::span<const uint8_t> header)
{
	...
	const uint32_t base_version = direct_version & 31u;
	const bool trusted_lan = base_version % 2 == 0;
	const bool lz4 = base_version >= 3;
	const bool safety = base_version >= 5;
	if (direct_block_active)
	{
...
void nxwarp_decoder::decode_unit(decode_job & job)
{
	...
	if (direct_block_active)
	{
		// Safety prefixes are decoded before every detail frame, even when never
		// displayed. They must not erase the separate detail-sample history.
		...
		std::optional<nxwarp_direct::motion_regions_header> motion_regions;
		const nxwarp_direct::motion_reference * reference = nullptr;
		if (nxwarp_direct::is_motion(payload))
		{
			const uint32_t motion_version = payload.size() >= 8 ? nxwarp_direct::read32(payload, 4) : 0;
			uint16_t reference_id = 0;
			if (motion_version == nxwarp_direct::motion_regions_version)
			{
				motion_regions = nxwarp_direct::parse_motion_regions_wire(payload);
				if (!direct_layout.motion_regions || !motion_regions)
				{
					host.report_frame_not_held(stream_index, job.frame_id, from_headset::nxwarp_frame_not_held::reason::refused);
					return;
				}
				reference_id = motion_regions->reference;
			}
			else
			{
			...
			if (!direct_layout.motion || job.safety || job.concealed ||
			    uint16_t(job.frame_id - reference_id) == 0 ||
			    uint16_t(job.frame_id - reference_id) > 8)
			{
				host.report_frame_not_held(stream_index, job.frame_id, from_headset::nxwarp_frame_not_held::reason::refused);
				return;
			}
			reference = direct_motion_references.find(reference_id);
			if (!reference)
			{
				...
				host.report_frame_not_held(stream_index, reference_id, from_headset::nxwarp_frame_not_held::reason::refused);
				host.report_frame_not_held(stream_index, job.frame_id, from_headset::nxwarp_frame_not_held::reason::refused);
			...
			payload = motion_regions ? motion_regions->body : motion->body;
		}
		if (nxwarp_direct::is_lz4(payload) || nxwarp_direct::is_zstd(payload))
		{
			const auto started = std::chrono::steady_clock::now();
			if (!direct_lz4 || !(nxwarp_direct::is_zstd(payload) ? (coded_layout.zstd && nxwarp_direct::decompress_zstd(coded_layout, payload, direct_unpacked)) : nxwarp_direct::decompress_lz4(coded_layout, payload, direct_unpacked)))
			{
				host.report_frame_not_held(stream_index, job.frame_id, from_headset::nxwarp_frame_not_held::reason::refused);
				return;
			}
		...
		if (motion || motion_regions)
		{
			...
			const bool restored = motion_regions
			                              ? nxwarp_direct::restore_motion_regions(coded_layout, reference->raw, reference->native,
			                                                                    direct_unpacked, motion_regions->vectors)
			                              : nxwarp_direct::restore_motion(coded_layout, reference->raw, reference->native,
			...
			payload = direct_unpacked;
			++direct_motion_frames;
			if (motion_regions) ++direct_motion_region_frames;
			if (direct_motion_frames % 180 == 1)
				spdlog::info("nxwarp[{}]: exact motion detail decoded {} (regional {}), missing references {}", stream_index, direct_motion_frames, direct_motion_region_frames, direct_motion_missing);
		}
		if (direct_lz4)
		{
		...
		const bool checker = nxwarp_direct::checker_frame(payload);
		...
		if (direct_layout.checkerboard && checker && !job.safety &&
		    direct_sample_history_frame != uint64_t(-1) &&
		    job.fb.frame_index == direct_sample_history_frame + 1 &&
		    job.fb.received_first_packet >= direct_sample_history_time &&
		    job.fb.received_first_packet - direct_sample_history_time <= 50'000'000)
		{
			history_frame = nxwarp_direct::parse_frame(coded_layout, direct_sample_history);
			if (history_frame)
			{
		...
		if (direct_layout.checkerboard && !job.concealed && !job.safety)
		{
			try
			{
				direct_sample_next_history.assign(payload.begin(), payload.end());
			}
			catch (...)
			{
				item->free = true;
		...
		if (direct_layout.checkerboard && !job.concealed && !job.safety)
		{
			direct_sample_history.swap(direct_sample_next_history);
			direct_sample_history_frame = job.fb.frame_index;
			direct_sample_history_time = job.fb.received_first_packet;
		}
		else if (!job.safety)
		{
			direct_sample_history.clear();
		...
		if (history_valid && (++direct_sample_history_used % 180 == 1))
			spdlog::info("nxwarp[{}]: checker previous-frame samples used {} times", stream_index, direct_sample_history_used);
		return;
	}
	if (planar_direct_active)
	{
